...
*   **Aesthetic & Design System:**
    *   **Images:** Managed via a hybrid model. Static assets like heroes and logos are discovered automatically via the `assets_manifest` (folder-based). Page-specific content (Services, Team, Categories) uses relative paths provided in Google Sheets.
...
*   **Core Files:**
    *   `index.html`, `services.html`, `meet-the-team.html`, `icon-service.html`, `reviews.html`: Page structures.
    ...
    *   **`js/features/` (Feature-Based Plugin Architecture):**
        ...
        *   `home-services.js`: Homepage-specific category card rendering.
        *   `portfolio.js`: Responsive portfolio image band/carousel.
        *   `reviews.js`: Testimonial expansion and rendering logic (supports shuffling and limiting).
...
*   **Atomic Source:** All site data (config, services, reviews, team, version, categories) is consolidated into a single `configs/site-data.json` file.
*   **Sync Workflow:** Data is updated from Google Sheets via the `sync-styleplanit.command` tool in the root directory. 
    ...
    *   **Audit Tool:** `scripts/diff_site_data.py` is an interactive 3-way resolution engine (Local vs. Sheets vs. Manual) used to identify and resolve discrepancies.
        ...
        *   **Efficiency:** Automatically clears previous reports and generates new CSVs using a custom `~~` delimiter to safely handle commas in multi-line text fields.
...
*   **Programs Selection (Homepage):**
    *   Displays dynamic category cards (images with text overlays) based on the `categories` table and the `showOnHomePage` flag.
    *   Title is configurable via `HOW_IT_WORKS_TITLE` (e.g., "PICK A JOURNEY").
*   **Portfolio Band:**
    *   Full-width horizontal carousel showcasing work images.
    *   Configurable with black padding for an editorial look.
*   **Integrated Featured Services:**
    ...
    *   **Exclusive Access (Icon Service):** A simplified, invitation-only collection (`icon-service.html`).
        *   **UI:** Renders only the "By Invitation Only" subtitle and a single, non-collapsible, auto-expanded service card. Uses a fixed-position background image with dark overlay for luxury impact.
...
*   **Smart Reviews:**
    *   **Homepage:** Randomly selects and displays 3 reviews to keep content fresh.
    *   **Full Reviews Page:** Dedicated page (`reviews.html`) displaying all testimonials.
    *   **Interaction:** Expandable cards with a luxury fade-out for long content and internal scroll-reset logic.
*   **Site Loader:** A full-screen branded overlay ensuring a seamless transition by waiting for dynamic data.

## 7. Development & Safety

*   **Version Control:** Active development follows a `feature/branching` model.
*   **Local Security:** `.env.asana` and sensitive tokens are strictly ignored by `.gitignore`. 
    *   **AI Protocol:** AI assistants must NEVER read or output the contents of `.env.asana`. To interact with the Asana API, use a subshell to source the token directly into the command (e.g., `(source .env.asana && curl -H "Authorization: Bearer $ASANA_PAT" ...)`).
    *   **Asana Workflow:**
        1.  **Project Identification:** Identify the "Style Plan-It Launch Plan" project (`gid: 1212636326772928`).
        2.  **Section Mapping:** `To do (1212636326772929)`, `Doing (1212623270704079)`, `Waiting (1212702244216478)`, `Done (1212623270704080)`.
    *   **Google Analytics:** Dynamic injection via `site-data.json` config.
    *   **Development Lifecycle:**
        1.  **Research & Strategy:** Map the codebase and propose a plan.
        2.  **Feature Branching:** Use dedicated `feature/` branches.
        3.  **Surgical Execution:** Apply precise changes following "Luxury Minimalist" aesthetics.
        4.  **User Verification (Mandatory):** AI assistants must NEVER commit changes without explicit confirmation.
        ...
        6.  **Pull Request & Merge:** Merge feature branch into `main` after verification.
...
*   **Infrastructure:** Production domain `https://styleplanit.com` is live.
*   **Recent Wins:** Redesigned homepage with "Pick a Journey" and Portfolio Band. Integrated randomized reviews and a dedicated Reviews page. Improved Data Audit tool with `~~` delimiter support for safer Google Sheets sync.
*   **Next Priorities:**
